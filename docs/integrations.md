...
- **Timeout**: 8 segundos (`.timeout(const Duration(seconds: 8))`), tanto na busca quanto no download da imagem em `_paletteFromUrl()`.
...
- **Cache positivo**: URL encontrada é armazenada permanentemente em `SharedPreferences` por `songId` (`artwork_url_cache`).
