...
- **Início de uma fila nova**: ao carregar a fila, o `just_audio` emite a sequência antes de aplicar o `initialIndex` (por um instante a faixa atual seria a primeira da lista). O handler ignora faixa atual diferente da pedida enquanto a fila carrega (`_pendingInitialIndex`), para tela, notificação e extração de cor não piscarem com outra música.
- **Edição da fila** (`QueueScreen`): remover funciona com ou sem shuffle (índice convertido da ordem efetiva para a original); a faixa atual não pode ser removida. Reordenar só com shuffle desligado. As edições valem só para a fila atual — tocar outra música recria a fila. O provider notifica a cada emissão de `queue` (rara: fila nova, shuffle ou edição). Validado no S25 em 2026-09-25: mover, remover (com e sem shuffle) e "próxima" seguindo a ordem editada.
- **Capa na notificação de mídia** (`PlayerProvider._loadNotificationArt`, a cada troca de faixa): usa a URL do iTunes quando houver (mesma regra da UI, `_resolveArtworkUrl`); senão a capa embutida (`queryArtwork`, 512 px, JPEG) gravada em `<systemTemp>/notification_art/<songId>.jpg` (no Android, `code_cache/`), mantendo só o arquivo da faixa atual. Entregue via `HammmAudioHandler.setArtUri()`, que guarda a URI por faixa e a reaplica a cada `mediaItem` emitido. Sem capa, a notificação fica sem imagem. Validado no S25 em 2026-09-25 nos dois caminhos.
...
- Validado no S25 em 2026-09-25 (arquivo apagado após o carregamento da biblioteca): `Source error` em ~0,5 s, um SnackBar e a próxima faixa começa a tocar. Se a fila tiver só a faixa com falha, ela continua como "atual" no mini player (parada).
...
- Resultado da busca por artwork é **cacheado permanentemente** por `songId` em `SharedPreferences` — uma vez encontrada, a URL não é buscada novamente, a não ser que a regra de escolha mude: o cache é versionado (`_artworkCacheVersion`) e, quando a versão salva difere, é descartado uma vez na abertura do app. A versão 2 descartou as capas aceitas sem validação de artista/título; cada uma é buscada de novo quando a música toca (até lá, a lista mostra a capa embutida ou o gradiente). Busca sem resultado compatível fica em cache negativo por 7 dias.
...
- "Negada permanentemente" vem só do resultado de `request()` (desde o `permission_handler` 13, `status` nunca reporta `permanentlyDenied` no Android); `status` é usado só para `isGranted` ao voltar ao app. Validado no S25 em 2026-09-25: 1ª negação mantém "Permitir Acesso", 2ª mostra "Abrir Configurações", e permissão concedida fora do app carrega a biblioteca ao voltar.
