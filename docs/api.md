...
| `position` / `duration` | `Duration` | Posição atual / duração da faixa |
...
| `sleepTimerRemaining` | `Duration?` | Tempo restante do sleep timer |
...
| `loadSongs()` | `Future<void>` | Consulta `MediaStore` via `on_audio_query`, filtra faixas ≤30s, aplica sort/filter |
...
| `skipToQueueItem(int index)` | `Future<void>` | Pula para item específico da fila |
...
| `toggleShuffle()` | `void` | Alterna shuffle (repassa a `just_audio`) |
...
Todos os métodos que alteram estado chamam `notifyListeners()` — o contrato de "resposta" desta API é reatividade via `ChangeNotifier`, não retorno de valor estruturado (a maioria retorna `Future<void>`).
